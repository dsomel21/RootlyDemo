...
class SlackIncidentDeclareWorkflowTest < ActionDispatch::IntegrationTest
  setup do
  ...
  test "successful declare command returns success when valid" do
  ...
  test "declare command workflow builds correct modal structure" do
  ...
  test "invalid command returns error message" do
    slack_params = build_slack_command_params(
      command: "/rootly",
  ...
  test "missing title shows help message" do
    # Mock the current_organization for test environment
    Slack::CommandsController.any_instance.stubs(:current_organization).returns(@organization)
  ...
  test "command router handles unknown commands" do
